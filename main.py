...
class CSV:
    ...
    def add_entry(cls, date, amount, category, description):
        ...
        def get_transactions(cls, start_date, end_date):
            ...
            print(f"Totaal Expense: ${total_expense:.2f}")
...
def add():
    CSV.initialize_csv()
    date = get_date("Enter the date of the transaction (mm-dd-yyyy) or enter today's date: ", allow_default=True)
...
def plot_transactions(df):
    ...
    plt.plot(expense_df, expense_df["amount"], label="expense", color="r")
    plt.xlabel("Date")
    plt.ylabel("Amount")
...
def main():
    while True:
        ...
        if choice == "1":
            ...
            if input("Do you want to see a plot? 9y/n) ").lower() == "y":
                plot_transactions(df)
        elif choice == "3":
            print("Exiting...")
            break 
        else: 
            print("Invaild choice. Enter 1,2,3.")
